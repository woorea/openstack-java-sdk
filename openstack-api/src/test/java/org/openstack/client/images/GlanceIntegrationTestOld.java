...
public class GlanceIntegrationTestOld extends AbstractOpenStackTest {
	...
	public void testListImagesAndDetails() throws OpenstackException {
		...
		ImagesService glance = client.images();
		List<GlanceImage> images = Lists.newArrayList(glance.getPublicEndpoint().get().getList());

		for (Image image : images) {
			Image details = glance.getPublicEndpoint().image(image.getId()).head();
	...
	public void testImageUploadAndDelete() throws Exception {
		...
		Image uploaded = glance.getPublicEndpoint().post(stream, imageLength, template);
		...
		List<GlanceImage> allImages = Lists.newArrayList(glance.getPublicEndpoint().get().getList());

		Image foundInAll = findImageById(allImages, uploaded.getId());
		assertNotNull(foundInAll);
		assertImageEquals(foundInAll, uploaded);

		Image details = glance.getPublicEndpoint().image(uploaded.getId()).head();
		assertImageEquals(details, uploaded);

		{
			InputStream is = glance.getPublicEndpoint().image(uploaded.getId()).openStream();
		...
		Response response = glance.getPublicEndpoint().image(uploaded.getId()).delete();

		for (int i = 0; i < 60; i++) {
			System.out.print(".");
			// Wait for up to 60 seconds for the image to be deleted
			allImages = Lists.newArrayList(glance.getPublicEndpoint().get().getList());
		...
		try {
			glance.getPublicEndpoint().image(uploaded.getId()).head();
			Assert.fail();
