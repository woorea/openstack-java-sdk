...
public class Test {
	...
	public static void main(String[] args) {
		...
		*/
		...
		serverForCreate.setName("eureka");
		serverForCreate.setFlavorRef("1");
		serverForCreate.setImageRef(imagesRepresentation.getList().get(1).getId());
		ServerRepresentation serverRepresentation = tenant.servers().create(serverForCreate);
		...
		*/
		...
		consolesResource.create();
		ConsoleList consoleList = consolesResource.list();
		for(Console console : consoleList.getList()) {
		...
		*/
		...
		/*
		KeyPairsResource keyPairsResource = compute.tenant(tenants.get(0).getId()).keyPairs();
		KeyPairList keyPairList = keyPairsResource.list();
		for(KeyPair keyPair : keyPairList.getList()) {
			System.out.println(keyPair);
		}
		*/
		/*
		SecurityGroupsResource securityGroupsResource = tenant.securityGroups();
		//SecurityGroup sg = new SecurityGroup();
		//sg.setName("test");
		//sg.setDescription("desc");
		//sg = securityGroupsResource.create(sg);
		SecurityGroupList securityGroupList = securityGroupsResource.list();
		for(SecurityGroup securityGroup : securityGroupList.getList()) {
			System.out.println(securityGroup);
		}
		*/
		/*
		SecurityGroupRulesResource rulesResource = securityGroupsResource.securityGroup(securityGroupList.getList().get(0).getId()).rules();
		...
		*/
		/*
		VolumesResource volumesResource = tenant.volumes();
		Volume vol = new Volume();
		vol.setSizeInGB(1);
		vol.setType("");
		VolumeList volumeList = volumesResource.list(false);
		for(Volume volumex : volumeList.getList()) {
			System.out.println(volumex);
		}
		*/
		
		/*
		ZonesResource zonesResource = tenant.zones();
		Zone zone = new Zone();
		zone.setName("dos");
		zonesResource.create(zone);
		ZoneList zoneList = zonesResource.detail();
		for(Zone zonex : zoneList.getList()) {
