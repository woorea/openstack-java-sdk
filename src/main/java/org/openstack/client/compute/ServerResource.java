...
public class ServerResource extends Resource {
	...
	public static class IpsResource extends Resource {
		...
		public String list(String networkId) {
			return client
					.resource(
							new StringBuilder(resource).append("/ips")
									.toString())
	...
	public ServerRepresentation update(Server server) {
		Server serverResult = client.resource(resource)
				.accept(MediaType.APPLICATION_XML).put(Server.class, server);
	 ...
	 * Reboot a server.
	 * 
	 * @param type either REBOOT_SOFT for a software-level reboot, or REBOOT_HARD for a virtual power cycle hard reboot.
	 */
	...
	public String pendingActions() {
		return client
				.resource(
						new StringBuilder(resource).append("/actions")
								.toString()).accept(MediaType.APPLICATION_XML)
				.get(String.class);
	}
	...
	public String virtualInterfaces() {
		return client
				.resource(
						new StringBuilder(resource).append(
								"/os-virtual-interfaces").toString())
				.accept(MediaType.APPLICATION_XML).get(String.class);
	}
	...
	public void diagnostics() {
		client.resource(
				new StringBuilder(resource).append("/diagnostics").toString())
				.accept(MediaType.APPLICATION_XML).get(String.class);
	}

	public IpsResource ips() {
		return new IpsResource(client, new StringBuilder(resource).append(
				"/ips").toString());
	}
	...
	private <T> T executeAction(Class<T> c, Object action) {
		return client
				.resource(
						new StringBuilder(resource).append("/action").toString())
	...
	public FloatingIpsResource floatingIps() {
		if (floatingIps == null) {
			floatingIps = new FloatingIpsResource(client, new StringBuilder(
					resource).append("/os-floating-ips").toString());
		}
		return floatingIps;
	}
	...
	public ConsolesResource consoles() {
		if (consoles == null) {
			consoles = new ConsolesResource(client, new StringBuilder(resource)
					.append("/consoles").toString());
		}
		return consoles;
	}
